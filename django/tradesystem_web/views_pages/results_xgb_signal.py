...
def results_xgb_signal(request):
    ...
    cache_key = (
        f'results_xgb_signal:{latest_trade_date or "none"}:{latest_horizon or "none"}:'
            ...
            if latest_trade_date is not None:
                sql = """
                    SELECT
                        ...
                        MAX(x.predicted_prob) AS predicted_prob,
                        MAX(x.predicted_label) AS predicted_label,
                        MAX(x.actual_label) AS actual_label,
                    ...
                    LEFT JOIN stock_prices_daily_xgb_signal x
                      ON x.code = l.code
                     AND x.trade_date = %s
                     AND x.horizon = %s
                ...
                sql = """
                    SELECT
                        ...
                        NULL AS predicted_prob,
                        NULL AS predicted_label,
    ...
    return render(request, "results_xgb_signal.html", context)

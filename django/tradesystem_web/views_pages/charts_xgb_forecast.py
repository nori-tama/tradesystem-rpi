...
from .common import calculate_exchange_business_days, shift_exchange_business_day


def stock_xgb_forecast_chart(request, code):
    with connection.cursor() as cursor:
        ...
        cursor.execute(
            """
            SELECT trade_date, horizon, model_version, trained_end_date
        ...
        latest_forecast_row = cursor.fetchone()

        forecast_rows = []
        latest_base_date = None
        latest_horizon = None
        latest_model_version = None
        trained_end_date = None
        base_close = None

        if latest_forecast_row is not None:
            latest_base_date = latest_forecast_row[0]
            latest_horizon = latest_forecast_row[1]
            latest_model_version = latest_forecast_row[2]
            trained_end_date = latest_forecast_row[3]
            ...
            cursor.execute(
                ...
                WHERE code = %s
                  AND trade_date = %s
                  AND model_version = %s
                  AND horizon IN (1, 2, 3, 4, 5)
                  AND predicted_close IS NOT NULL
                ORDER BY horizon
                """,
                [code, latest_base_date, latest_model_version],
            )
            forecast_rows = cursor.fetchall()

            if forecast_rows:
                base_close = float(forecast_rows[0][5]) if forecast_rows[0][5] is not None else None
    ...
    for horizon, predicted_close, _actual_close, _predicted_return, _actual_return, _row_base_close in forecast_rows:
        if predicted_close is None:
            continue
        target_trade_date = shift_exchange_business_day(latest_base_date, int(horizon))
    ...
    if latest_base_date is not None:
        future_end = shift_exchange_business_day(latest_base_date, 5)
        ...
        if future_end is not None and (timeline_end is None or future_end > timeline_end):
            timeline_end = future_end
    ...
    if timeline_start is not None and timeline_end is not None:
        ...
        if latest_base_date is not None:
            ...
            for horizon in [1, 2, 3, 4, 5]:
                target_day = shift_exchange_business_day(latest_base_date, horizon)
                if target_day is not None:
                    forecast_day_set.add(target_day)
